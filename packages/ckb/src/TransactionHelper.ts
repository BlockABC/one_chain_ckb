...
import { IUTXOFromToParam, IUTXOInput, IUTXOOutput, IUTXOUnspent, OneChainError } from '@onechain/core'
...
export class TransactionHelper {
  ...
  readonly minOutputValue = MIN_CELL_CAPACITY
  readonly txPartialSize = TX_SIZE
   ...
   */
  isUnspentsEnough (
    { unspents, tos, fee, autoFix }:
    { unspents: IUTXOUnspent[], tos: IUTXOFromToParam[], fee: Decimal, autoFix: boolean }
  ): void {
  ...
  selectUnspentsByOrder (
    { unspents, tos, changes, fee = new Decimal(0) }:
    { unspents: IUTXOUnspent[], tos: IUTXOFromToParam[], changes: IUTXOOutput[], fee?: Decimal }
  ):
    {
      inputs: IUTXOInput[],
      outputs: IUTXOOutput[],
    ...
    const outputs: IUTXOOutput[] = tos.map((to: IUTXOOutput): IUTXOOutput => {
      return {
        address: to.address,
        value: to.value,
      }
  ...
  selectUnspentsByAutofix (
    ...
    let outputs: IUTXOOutput[]

    if (inputValue.gt(outputValue)) {
      outputs = tos.map((to: IUTXOOutput): IUTXOOutput => {
        return {
          address: to.address,
          value: to.value,
        }
   ...
   */
  calcInputOutput (
    { unspents, tos, changes, fee = new Decimal(0), autoFix = false }:
    { unspents: IUTXOUnspent[], tos: IUTXOFromToParam[], changes: IUTXOOutput[], fee?: Decimal, autoFix?: boolean }
  ):
    {
      inputs: IUTXOInput[],
      outputs: IUTXOOutput[],
  ...
  estimateSize (
    ...
    const size =
      (
        LENGTH_BYTE_SIZE + SERIALIZED_OFFSET_BYTESIZE * 6 +
